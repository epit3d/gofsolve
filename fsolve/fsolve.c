  ...
  if (gnorm != 0.0)
  {
    ...
    */
    if (sgnorm < delta)
    {
      ...
      temp = temp - (delta / qnorm) * (sgnorm / delta) * (sgnorm / delta) + sqrt(pow(temp - (delta / qnorm), 2) + (1.0 - (delta / qnorm) * (delta / qnorm)) * (1.0 - (sgnorm / delta) * (sgnorm / delta)));
...
void fdjac1_wrapped(uintptr_t handle,
            int n, double x[], double fvec[], double fjac[], int ldfjac,
            int ml, int mu, double epsfcn, double wa1[], double wa2[])
{
  double eps;
...
int fsolve_wrapped(uintptr_t handle, int n, double x[],
           double fvec[], double tol)
{
  double epsfcn;
  double factor;
  ...
  */
  ...
  info = hybrd_wrapped(handle, n, x, fvec, xtol, maxfev, ml, mu, epsfcn, wa, mode,
               factor, nfev, wa + index, n, wa + 6 * n, lr,
               wa + n, wa + 2 * n, wa + 3 * n, wa + 4 * n, wa + 5 * n);
...
int hybrd_wrapped(uintptr_t handle,
          int n, double x[],
          double fvec[], double xtol, int maxfev, int ml, int mu, double epsfcn,
          double diag[], int mode, double factor, int nfev,
          double fjac[], int ldfjac, double r[], int lr, double qtf[], double wa1[],
  ...
  */
  for (;;)
  {
    ...
    /*
      Calculate the jacobian matrix.
    */
    fdjac1_wrapped(handle, n, x, fvec, fjac, ldfjac, ml, mu, epsfcn, wa1, wa2);
